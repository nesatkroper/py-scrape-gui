...
class SystemMonitorApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.title("Nun System & Network Monitor")
    ...
    def setup_ui(self):
        ...
        monitor_card = ctk.CTkFrame(self, corner_radius=8)
        monitor_card.grid(row=1, column=0, padx=10, pady=5, sticky="nsew")
        ...
        for i, (metric, color) in enumerate(metrics):
            ...
            label = ctk.CTkLabel(
                monitor_card, text=f"{metric}: 0.0%", font=ctk.CTkFont(size=11)
            )
            label.grid(row=i * 2 + 1, column=0, padx=10, pady=(5, 2), sticky="w")
            ...
            progress.grid(row=i * 2 + 2, column=0, padx=10, pady=(0, 5), sticky="ew")
            progress.set(0)
            self.progress_bars[metric] = progress

        net_frame = ctk.CTkFrame(monitor_card, fg_color="transparent")
        net_frame.grid(row=9, column=0, padx=10, pady=(5, 10), sticky="ew")
        net_frame.grid_columnconfigure((0, 1), weight=1)

        self.labels["Net Download"] = ctk.CTkLabel(
            net_frame, text="Download: 0.00 KB/s", font=ctk.CTkFont(size=11)
        )
        self.labels["Net Download"].grid(row=0, column=0, sticky="w")

        self.labels["Net Upload"] = ctk.CTkLabel(
            net_frame, text="Upload: 0.00 KB/s", font=ctk.CTkFont(size=11)
        )
        self.labels["Net Upload"].grid(row=0, column=1, sticky="e")

        speed_card = ctk.CTkFrame(self, corner_radius=8)
        speed_card.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        ...
        self.speed_test_button.grid(row=1, column=0, padx=20, pady=(5, 5), sticky="ew")
        ...
        self.speed_results.grid(row=2, column=0, padx=20, pady=(5, 10), sticky="ew")
